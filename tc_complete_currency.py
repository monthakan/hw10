...
class CurrencyTools:
    ...
    def convert(self, amount: float, base: str, quote: str) -> Dict[str, Any]:
        ...
        if f"{base}->{quote}" in RATE_TABLE: # check if rate exists in RATE_TABLE
            rate = RATE_TABLE[f"{base}->{quote}"] # get rate from RATE_TABLE
    ...
    def get_schemas(cls) -> List[dict]:
        ...
        return [
            ...
            { #Schema นี้บอก input เท่านั้น -> บอก llm ว่า tool convert ต้องการอะไรบ้าง
                "name": "convert",
                "description" : "Convert amount from base to quote using fixed RATE_TABLE",
                "parameters" : {
                    "type" : "object",
                    "properties" : { #ใส่ properties ให้ครบตามที่ฟังก์ชัน convert ต้องการ
                        "amount" : {"type": "number"},
                        "base" : {"type": "string"},
                        "quote" : {"type" : "string"},
                    },
...
if __name__ == "__main__":
    ...
    # JSON printout
    print(json.dumps(tools.convert(100, "USD", "THB"), indent=2))  # direct call to convert() to see result structure
    print(json.dumps(tools.convert(250, "THB", "EUR"), indent=2))  # direct call to convert() to see result structure
